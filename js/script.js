...
(function($){
  ...
  $('body').on('click', function(){
    ...
    if ($('#' + id).length){
      ...
      var safeHtml = [
        '<div id="' + encodeAttr(id) + '" class="article-share-box">',
          '<div class="article-share-links">',
            '<a href="https://twitter.com/intent/tweet?url=' + encodeAttr(encodedUrl) + '&text=' + encodeAttr(title) + '" class="article-share-twitter" target="_blank" title="Twitter"></a>',
            '<a href="https://www.facebook.com/sharer/sharer.php?u=' + encodeAttr(encodedUrl) + '" class="article-share-facebook" target="_blank" title="Facebook"></a>',
            '<a href="https://www.linkedin.com/shareArticle?mini=true&url=' + encodeAttr(encodedUrl) + '&title=' + encodeAttr(title) + '&summary=' + encodeAttr(summary) + '" class="article-share-linkedin" target="_blank" title="LinkedIn"></a>',
          '</div>',
        '</div>'
      ].join('');
...
//      if (alt) $(this).after('<span class="caption">' + alt + '</span>');
      $(this).wrap('<a href="' + encodeAttr(target) + '" title="' + encodeAttr(alt) + '" class="fancybox"></a>');
    });
  ...
  $("[href^='#']").not("[href~='#']").click(function(evt){
    evt.preventDefault();
    var obj = $(this),
      getHref = decodeURIComponent(obj.attr("href").split("#")[1]),
      target = $("[id='" + getHref + "']");
  
    if (target.length) {
      var offsetSize = target.offset().top - navHeight;
      $("html, body").animate({ scrollTop: offsetSize }, 200);
    } else {
      console.warn("Target element not found:", getHref);
    }
  });
